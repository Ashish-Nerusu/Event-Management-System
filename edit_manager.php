...
if (isset($_GET['user_id'])) {
    ...
    $query = "SELECT username, email FROM managers WHERE user_id = ?";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $update_query = "UPDATE managers SET username = ?, email = ? WHERE user_id = ?";
        $stmt = $conn->prepare($update_query);
        $stmt->bind_param("ssi", $new_username, $new_email, $user_id);

        if ($stmt->execute()) {
            // Redirect back to user management page with success message
            header("Location: manager_management.php?message=User updated successfully");
            exit();
        } else {
            $error_message = "Failed to update manager. Please try again.";
        }
...
<body>
    ...
    <form method="post">
        ...
        <button type="button" onclick="window.location.href='manager_management.php'">Cancel</button>
    </form>
</body>
</html>
